/**
 * normalize-db.mjs
 * Purpose: normalize db.json into a clean, load-ready seed contract.
 * Input: db.json (repo root)
 * Output: db.normalized.seed.json (repo root)
 */

import fs from 'node:fs';
import path from 'node:path';

const inputPath = path.resolve('db.json');
const loadReadyPath = path.resolve('db.normalized.seed.json');
...
for (const plant of Object.values(fixed.plants || {})) {
  if (plant.wateringFrequencyDays && !plant.careFrequencyPerWeek) {
    const v = Number(plant.wateringFrequencyDays);
    if (v > 0) {
      plant.careFrequencyPerWeek = Math.max(1, Math.round((7 / v) * 100) / 100);
    }
    delete plant.wateringFrequencyDays;
  }
