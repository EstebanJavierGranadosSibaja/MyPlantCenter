...
export function useUserProfile(userId: string) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      else setError(res.message ?? 'Error al cargar el perfil.');
    } catch {
  ...
  const updatePrivacy = useCallback(async (dto: UpdatePrivacyDTO) => {
    ...
    try {
      ...
      if (!res.success) {
        setProfile(anterior);
        setError(res.message ?? 'Error al actualizar privacidad.');
      }
    } catch {
      setProfile(anterior);
      setError('Error de conexión. Intenta de nuevo.');
  ...
  const updateNotifications = useCallback(async (dto: UpdateNotificationsDTO) => {
    ...
    try {
      ...
      if (!res.success) {
        setProfile(anterior);
        setError(res.message ?? 'Error al actualizar notificaciones.');
      }
    } catch {
      setProfile(anterior);
      setError('Error de conexión. Intenta de nuevo.');
