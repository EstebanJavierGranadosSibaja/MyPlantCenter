...
│   │   └── ProfileView/             # Módulo de perfil (pantalla + estados + tabs)
│   │       ├── components/          # Header, estados especiales y composición
│   │       └── profile/             # Secciones de dominio del perfil
│   │           ├── ProfileHero/
│   │           ├── ProfileTabs/
│   │           ├── TabPerfil/
│   │           ├── TabCategorias/
...
Organismos (profile/)→ ProfileHero, ProfileTabs
Páginas (screens/)   → Dashboard, Explore, ProfileView
```
...
│   ├── "Inicio"      → Pantalla de inicio     (icono: home)
│   └── "Explorar"    → Pantalla de búsqueda   (icono: search)
│
├── "Perfil"          → Perfil propio (tab)
└── "UserProfile"     → Perfil de tercero       (pantalla completa)
```
